...
1. Which is the probability of a pea being yellow? And of being green? Which term is this probability in the Bayes' theorem?
...
3. And what is the probability of this yellow pea being $\mathtt{Yy}$?   
...
1. Which is the probability that someone is COVID+, i.e., $P(COVID+)$? And COVID-?   
...
2. Now, if we change the probability of independently observing the nucleotides to $P(C) = P(G) = 0.2$ and $P(A) = 0.1$ and $P(T) = 0.5$, what is the probability of observing this sequence? And of sequence `AATCGGA`?   
...
```{r}
optimise(lnL_binom, interval = c(0,1), k = 19, n = 62, maximum = T)
```
...
optim(par = c(50,50), fn = lnL_norm, control = list(fnscale = -1), D = weight)
```
